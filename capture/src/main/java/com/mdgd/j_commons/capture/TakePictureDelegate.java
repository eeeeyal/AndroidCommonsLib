package com.mdgd.j_commons.capture;
...
public class TakePictureDelegate {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PRC_CAPTURE && permissionsGranted) makePicture();
        else if (requestCode == PRC_SELECT && permissionsGranted) selectFromGallery();
    }

    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode != Activity.RESULT_OK) return;

        if (requestCode == RC_CAPTURE) fetchNewImage(data);
        else if (requestCode == RC_SELECT && data != null && data.getData() != null) {
            if (funcOnImageObtained != null) funcOnImageObtained.onImageObtained(data.getData());
        }
    }
    ...
    private File getSavePictureFile() {
        ...
        } else if (!TextUtils.isEmpty(imagePath)) return new File(imagePath);
        else return new File(Environment.getDataDirectory() + "/Pictures");
    }
